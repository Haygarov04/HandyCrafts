...
import { money, productionDays } from "@/lib/catalog";
...
export default function CartPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setError("");
    if (!agree) {
      setError("Потвърди общите условия.");
      return;
    }
    setSending(true);
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Поръчката не мина.");
      cart.clear();
      router.push(`/thanks?n=${encodeURIComponent(data.number)}`);
    } catch (issue) {
      setError(issue instanceof Error ? issue.message : "Поръчката не мина.");
      setSending(false);
    }
  }
  ...
  if (cart.ready && cart.items.length === 0) {
    return (
      <div className="mx-auto flex min-h-[70vh] max-w-lg flex-col items-center justify-center px-4 pt-28 text-center">
        <h1 className="text-3xl sm:text-4xl">Количката е празна</h1>
        <p className="mt-4 text-ink/60">Качи снимка, виж визуализацията и я добави тук.</p>
        <Link href="/studio" className="mt-8 rounded-full bg-ember px-7 py-3.5 font-semibold text-ink">
          Създай фигурка
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pb-24 pt-32 sm:px-6 sm:pt-36">
      <h1 className="text-3xl sm:text-5xl">Поръчка</h1>
      <p className="mt-3 text-ink/60">Плащаш с наложен платеж при получаване. Ще ти се обадим, за да потвърдим.</p>
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        <form onSubmit={submit} className="order-2 space-y-8 lg:order-1">
          <fieldset className="rounded-[2rem] bg-white p-6 sm:p-8">
            <legend className="float-left mb-5 w-full font-display text-xl">Твоите данни</legend>
            <div className="grid clear-both gap-3 sm:grid-cols-2">
              <input required autoComplete="name" placeholder="Име и фамилия" value={form.name} onChange={set("name")} className={field} />
              <input required type="tel" autoComplete="tel" placeholder="Телефон" value={form.phone} onChange={set("phone")} className={field} />
              <input type="email" autoComplete="email" placeholder="Имейл (за потвърждение)" value={form.email} onChange={set("email")} className={`${field} sm:col-span-2`} />
            </div>
            <input
              tabIndex={-1}
              autoComplete="off"
          ...
          <fieldset className="rounded-[2rem] bg-white p-6 sm:p-8">
            <legend className="float-left mb-5 w-full font-display text-xl">Доставка</legend>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-[0.8fr_1.2fr]">
              <input required autoComplete="address-level2" placeholder="Град" value={form.city} onChange={set("city")} className={field} />
              <input
                required
                autoComplete={form.delivery === "address" ? "street-address" : "off"}
                placeholder={form.delivery === "address" ? "Улица, номер, вход, етаж" : "Кой офис (име или адрес)"}
                value={form.address}
            ...
            <textarea
              placeholder="Бележка (по желание) — напр. трябва ми до 14 февруари"
              value={form.note}
              onChange={set("note")}
              maxLength={600}
          ...
          <div className="rounded-[2rem] bg-white p-6 sm:p-8">
            <p className="font-display text-xl">Плащане</p>
            <div className="mt-4 flex items-center gap-3 rounded-2xl border-2 border-ember bg-ember/5 px-4 py-3">
              ...
              <span>
                <span className="block text-sm font-semibold">Наложен платеж</span>
                <span className="block text-xs text-ink/55">Плащаш на куриера при получаване</span>
              </span>
            </div>
            <label className="mt-5 flex items-start gap-3 text-sm text-ink/70">
              ...
              <span>
                Съгласен съм с{" "}
                <Link href="/terms" target="_blank" className="underline decoration-ember underline-offset-4">
                  общите условия
                </Link>{" "}
                и разбирам, че фигурката се изработва по моя снимка.
              </span>
            </label>
            ...
            >
              {sending ? "Изпращаме…" : `Поръчай — ${money(cart.total)}`}
            </button>
          </div>
        </form>
        ...
        <aside className="order-1 lg:order-2">
          <div className="rounded-[2rem] bg-white p-6 sm:p-8 lg:sticky lg:top-32">
            <p className="font-display text-xl">В количката</p>
            ...
            <dl className="mt-6 space-y-2 border-t border-ink/10 pt-5 text-sm">
              <div className="flex justify-between">
                <dt className="text-ink/60">Продукти</dt>
                <dd>{money(cart.total)}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-ink/60">Доставка</dt>
                <dd>по тарифа на куриера</dd>
              </div>
            ...
            <p className="mt-4 text-xs leading-5 text-ink/50">Изработка и изпращане: {productionDays}.</p>
            <Link href="/studio" className="mt-4 inline-block text-sm font-semibold underline decoration-ember underline-offset-4">
              + Добави още една фигурка
            </Link>
          </div>
        </aside>
      </div>
